...
const findEmployeeOption = (options, value) => {
  ...
  const extractedCode = match
    ? String(match[2] || "")
        ...
        .toLowerCase()
    : "";
  const normalizedName = (match ? String(match[1] || "") : parsed)
    .trim()
    .toLowerCase();
  ...
  return (
    options.find((item) => {
      ...
      const optionName = String(item.name || "")
        .trim()
        .toLowerCase();
      const optionLabel = String(item.label || "")
        .trim()
        .toLowerCase();
...
function WarningLetterManager({ pageTitle, subtitle, apiClient }) {
  ...
  const selectedEmployee = useMemo(() => {
    return (
      employees.find(
        (item) => String(item.employee_id) === String(form.employee_id),
      ) || null
    );
  ...
  const isEvaluasiHR = useMemo(() => {
    return selectedEmployee
      ? String(selectedEmployee.alpha_sanction_level || "").toLowerCase() ===
          "evaluasi_hr"
      : false;
  }, [selectedEmployee]);
  ...
  const yearOptions = useMemo(() => {
    ...
    impactedEmployees.forEach((item) => {
      const parts = getDateParts(
        item.latest_alpha_date || item.violation_date_start,
      );
  ...
  const filteredImpactedEmployees = useMemo(() => {
    ...
    return impactedEmployees.filter((item) => {
      ...
      const matchesLevel = impactedFilter.sanctionLevel
        ? sanctionLevel === impactedFilter.sanctionLevel
        : true;
      ...
      const matchesMonth = impactedFilter.month
        ? dateParts?.month === impactedFilter.month
        : true;
      const matchesYear = impactedFilter.year
        ? dateParts?.year === impactedFilter.year
        : true;
  ...
  const filteredLetters = useMemo(() => {
    ...
    return letters
      .filter((item) => {
        ...
        const matchesLevel = historyFilter.spLevel
          ? level === historyFilter.spLevel
          : true;
        if (!matchesLevel) return false;
        ...
        const matchesMonth = historyFilter.month
          ? dateParts?.month === historyFilter.month
          : true;
        const matchesYear = historyFilter.year
          ? dateParts?.year === historyFilter.year
          : true;
        if (!matchesMonth || !matchesYear) return false;
      ...
      .slice()
      .sort((a, b) => {
        const issuedA = new Date(
          a?.issued_date || a?.created_at || 0,
        ).getTime();
        const issuedB = new Date(
          b?.issued_date || b?.created_at || 0,
        ).getTime();
  ...
  const letterPreviewText = useMemo(() => {
    ...
    if (isEvaluasiHRPreview) {
      ...
      const evalDay = new Date(
        form.evaluation_date + "T00:00:00",
...
Sehubungan dengan catatan pelanggaran kedisiplinan kehadiran yang telah mencapai tahap Surat Peringatan III (SP3), Saudara diminta untuk menghadiri sesi Evaluasi HR.

Evaluasi ini bertujuan untuk meninjau riwayat kehadiran dan memberikan kesempatan kepada Saudara untuk menyampaikan klarifikasi atas pelanggaran yang terjadi.
...
Surabaya, ${issuedDateText}`;
    ...
    return `PT OTAK KANAN\nGraha Pena, Ruang 1503, Jl. Ahmad Yani No.88, Ketintang, Kec. Gayungan, Surabaya, Jawa Timur 60234\n\n${spTitle}\nNo: ${letterNumber}\n\nDiberikan kepada:\nNama       : ${employeeName}\nDepartemen : ${departmentName}\nPosisi     : ${positionName}\nNPWP       : ${npwp}\n\n${violationReason}\n\nBerdasarkan hal tersebut, perusahaan menerbitkan ${spTitle}.\nSaudara diharapkan tidak mengulangi pelanggaran serupa dan meningkatkan\nkedisiplinan kerja.\n\nSurat peringatan ini berlaku selama 6 bulan sejak tanggal diterbitkan.\n\nSurabaya, ${issuedDateText}\n\n${signedTitle}\n(${signedName})`;
  ...
  useEffect(() => {
    if (!selectedEmployee) return;
    const suggestedLevel = String(
      selectedEmployee.alpha_sanction_level || "",
    ).toLowerCase();
    const nextLevel = ["sp1", "sp2", "sp3", "evaluasi_hr"].includes(
      suggestedLevel,
    )
      ? suggestedLevel
      : "sp1";
    setForm((prev) => ({
      ...
      sp_level: nextLevel,
      violation_date:
        selectedEmployee.violation_date_label ||
        toDateInputValue(selectedEmployee.latest_alpha_date || ""),
    }));
  }, [selectedEmployee]);
  ...
  const handleSelectEmployee = (employee) => {
    const suggestedLevel = String(
      employee?.alpha_sanction_level || "",
    ).toLowerCase();
    const mappedLevel = ["sp1", "sp2", "sp3", "evaluasi_hr"].includes(
      suggestedLevel,
    )
      ? suggestedLevel
      : "sp1";
    ...
    setForm((prev) => ({
      ...
      violation_date:
        employee.violation_date_label ||
        toDateInputValue(employee.latest_alpha_date || ""),
  ...
  const handleCreate = async (event) => {
    ...
    try {
      ...
      const previewElement = document.getElementById("warning-letter-pdf");
      if (previewElement && letterPreviewText) {
        const pdfBlob = await buildWarningLetterPdfBlob();
        const formData = new FormData();
        formData.append("file", pdfBlob, `warning_letter_${Date.now()}.pdf`);

        let uploadRes;
        try {
          uploadRes = await axios.post("/api/upload-warning-letter", formData, {
          ...
          uploadRes = await axios.post("/api/upload-invitation", formData, {
            headers: { "Content-Type": "multipart/form-data" },
          });
        }

        payload.file_path = uploadRes?.data?.path || uploadRes?.data?.url || "";
      }
  ...
  const buildWarningLetterPdfBlob = async () => {
    ...
    const element = document.getElementById("warning-letter-pdf");
    if (!element) {
      throw new Error("Preview surat tidak ditemukan");
    }

    const opt = {
      margin: 0,
      filename: `warning-letter-${Date.now()}.pdf`,
      image: {
        type: "jpeg",
        quality: 1,
      },
  ...
  const handleGeneratePdf = async () => {
    try {
      ...
      a.href = url;
      a.download = `warning-letter-${Date.now()}.pdf`;
  ...
  const handlePrint = (letter) => {
    ...
    popup.document.write(`
            <html>
                ...
                <body>
                    <div class="sheet">
                    <div class="letter-content">${isHtml ? rawContent : String(rawContent).replace(/\n/g, "<br/>")}</div>
                    </div>
                </body>
            </html>
        `);
      ...
      <TitleCard title="Pegawai Terkena SP" topMargin="mt-0">
        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-3 mb-4">
          <input
            ...
            onChange={(event) =>
              handleImpactedEmployeeInputChange(event.target.value)
            }
          />
          ...
          <select
            ...
            onChange={(event) =>
              setImpactedFilter((prev) => ({
                ...prev,
                sanctionLevel: event.target.value,
              }))
            }
          >
              ...
              {filteredImpactedEmployees.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center opacity-60">
                    Belum ada pegawai yang terkena SP
                  </td>
                </tr>
              ) : (
                filteredImpactedEmployees.map((item) => (
                  <tr key={item.employee_id}>
                    ...
                    <td>
                      {alphaSanctionLabelMap[
                        String(item.alpha_sanction_level || "none")
                      ] || "-"}
                    </td>
                    <td>{Number(item.alpha_consecutive_days || 0)} hari</td>
                    <td>{Number(item.alpha_accumulated_days || 0)} hari</td>
                    <td>
                      {item.violation_date_label ||
                        formatDate(item.latest_alpha_date)}
                    </td>
                    <td>
                      <button
                      ...
                      >
                        {String(
                          item.alpha_sanction_level || "",
                        ).toLowerCase() === "evaluasi_hr"
                          ? "Buat Undangan Evaluasi"
                          : "Buat SP"}
                      </button>
                    </td>
                  </tr>
            ...
            >
              <label className="form-control md:col-span-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    handleInput("employee_id", event.target.value)
                  }
                  required
                >
                  {impactedEmployees.map((item) => {
                    ...
                    return (
                      <option key={item.employee_id} value={item.employee_id}>
                        ...
                        {item.position_name || "-"} | SP saat ini:{" "}
                        {sanctionLabel}
                      </option>
                    );
                  })}
              ...
              {!isEvaluasiHR && (
                <label className="form-control">
                  ...
                  <select
                    ...
                    value={form.sp_level}
                    onChange={(event) =>
                      handleInput("sp_level", event.target.value)
                    }
                    required
                  >
                    <option value="sp1">SP1</option>
              ...
              <label className="form-control">
                ...
                <input
                  ...
                  onChange={(event) =>
                    handleInput("issued_date", event.target.value)
                  }
                  required
                />
              </label>
                ...
                <>
                  <label className="form-control">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        handleInput("evaluation_date", event.target.value)
                      }
                      required
                    />
                  </label>
                  <label className="form-control">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        handleInput("evaluation_time", event.target.value)
                      }
                      required
                    />
                  </label>
                  <label className="form-control md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        handleInput("evaluation_place", event.target.value)
                      }
              ...
              <div className="form-control md:col-span-2">
                ...
                <div className="alert alert-info mt-1 text-sm">
                  Otomatis menggunakan posisi dan nama lengkap akun yang membuat
                  surat.
                </div>
              </div>
              ...
              {!isEvaluasiHR && (
                <label className="form-control md:col-span-2">
                  <span className="label-text">
                    Alasan/Penjelasan Pelanggaran (opsional)
                  </span>
                  <textarea
                ...
                <div className="md:col-span-2">
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        Surabaya, {getTodayIndo(form.issued_date)}
                      </div>

                      {/* ISI SURAT */}
                      <div>
                        {(() => {
                          const lines = letterPreviewText
                            .replace(/^PT OTAK KANAN[\s\S]*?60234\n\n/, "")
                            .split("\n");
                          ...
                          return (
                            <>
                              {cleanLines.map((line, index) => {
                                ...
                                // JUDUL
                                if (
                                  trimmed.includes("SURAT PERINGATAN") ||
                                  trimmed.includes("UNDANGAN EVALUASI HR")
                                ) {
                                  return (
                                    <div
                                      key={index}
                                ...
                                if (
                                  ...
                                  trimmed.startsWith("Departemen") ||
                                  trimmed.startsWith("Posisi") ||
                                  trimmed.startsWith("NPWP") ||
                                  trimmed.startsWith("Hari/Tanggal") ||
                                ...
                                // TANDA TANGAN
                                if (
                                  trimmed.includes("[Posisi akun pembuat") ||
                                  trimmed.includes("[Nama lengkap akun pembuat")
                                ) {
                                  return null;
                                }

                                // PENUTUP HORMAT
                                if (trimmed.startsWith("Surabaya,")) {
                                  return (
                                    <div
                                      key={index}
                                      style={{
        ...
        <TitleCard title="Riwayat Surat Peringatan" topMargin="mt-0">
          <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-3 mb-4">
            <input
              ...
              onChange={(event) =>
                handleHistoryEmployeeInputChange(event.target.value)
              }
            />
                    ...
                    <tr key={item.id}>
                      ...
                      <td>
                        <span className="badge badge-outline">
                          {spLabelMap[item.sp_level] || item.sp_level}
                        </span>
                      </td>
