...
export default function Pagination({ page, totalPages, onChangePage, itemsPerPage = 10, disabled = false }) {
  const handleFirstPage = () => onChangePage(1)
  const handlePrevPage = () => onChangePage(Math.max(1, page - 1))
  const handleNextPage = () => onChangePage(Math.min(page + 1, totalPages || 1))
  const handleLastPage = () => onChangePage(totalPages || 1)

  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
      <div className="text-sm opacity-70">
        Menampilkan halaman <span className="font-semibold">{page}</span> dari <span className="font-semibold">{totalPages}</span> halaman
        <span className="ml-2 text-xs opacity-60">({itemsPerPage} item per halaman)</span>
      </div>
      <div className="join">
        <button
