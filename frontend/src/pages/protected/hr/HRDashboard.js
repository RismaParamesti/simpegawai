...
const formatViolationCounts = (item) => {
  ...
  if (alphaConsecutive > 0 || alphaAccumulated > 0) {
    parts.push(
      `Alpha ${alphaConsecutive} berturut / ${alphaAccumulated} akumulasi`,
    );
  }
  if (lateConsecutive > 0 || lateAccumulated > 0) {
    parts.push(
      `Telat ${lateConsecutive} berturut / ${lateAccumulated} akumulasi`,
    );
  }
...
const DashboardSection = ({
  ...
  <div
    className={`rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-[0_16px_44px_rgba(15,23,42,0.05)] ${className}`}
  >
...
function HRDashboard() {
        ...
        <div className="rounded-2xl border border-red-200 dark:border-red-700 bg-red-50 dark:bg-red-900/20 p-5">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h2 className="font-extrabold text-red-700 dark:text-red-200">
                Dashboard HR gagal dimuat
              </h2>
              <p className="mt-1 text-sm font-medium text-red-600/90 dark:text-red-300">
                {error}
              </p>
              <button
  ...
  const reimbursementMonthlyHistory =
    Number(reimbursementValidation.stats?.total || 0) ||
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: Number(
        reimbursementValidation.stats?.pending_validation_count || 0,
      ),
      icon: WalletIcon,
  ...
  const attendanceCards = [
    {
      ...
      status: "hadir",
      box: "bg-emerald-50 border-emerald-200 text-emerald-700 dark:border-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300",
    },
    {
      label: "Terlambat",
      value: attendanceToday.late || 0,
      status: "late",
      box: "bg-amber-50 border-amber-200 text-amber-700 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300",
    },
    {
      label: "Sakit",
      value: attendanceToday.sakit || 0,
      status: "sakit",
      box: "bg-blue-50 border-blue-200 text-blue-700 dark:border-blue-700 dark:bg-blue-900/20 dark:text-blue-300",
    },
    {
      label: "Izin",
      value: attendanceToday.izin || 0,
      status: "izin",
      box: "bg-violet-50 border-violet-200 text-violet-700 dark:border-violet-700 dark:bg-violet-900/20 dark:text-violet-300",
    },
    {
      label: "Alpha",
      value: attendanceToday.alpha || 0,
      status: "alpha",
      box: "bg-red-50 border-red-200 text-red-700 dark:border-red-700 dark:bg-red-900/20 dark:text-red-300",
    },
  ];
  ...
  const summaryCards = [
    {
      ...
      box: "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300",
    },
    {
      label: "Total Terlambat",
      value: attendanceSummary.total_late || 0,
      box: "border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300",
    },
    {
      label: "Sakit",
      value: attendanceSummary.sakit || 0,
      box: "border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-700 dark:bg-blue-900/20 dark:text-blue-300",
    },
    {
      label: "Izin",
      value: attendanceSummary.izin || 0,
      box: "border-violet-200 bg-violet-50 text-violet-700 dark:border-violet-700 dark:bg-violet-900/20 dark:text-violet-300",
    },
    {
      label: "Alpha",
      value: attendanceSummary.alpha || 0,
      box: "border-red-200 bg-red-50 text-red-700 dark:border-red-700 dark:bg-red-900/20 dark:text-red-300",
    },
    {
      label: "Rata-rata Terlambat",
      value: `${(parseFloat(attendanceSummary.avg_late_minutes) || 0).toFixed(0)} min`,
      box: "border-slate-200 bg-slate-50 text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200",
    },
  ];
    ...
    <div className="mt-4 overflow-hidden rounded-[1.5rem] border border-slate-200/80 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-[0_20px_70px_rgba(15,23,42,0.07)] sm:p-7">
      <div className="space-y-6">
        <div className="relative min-h-[130px] overflow-hidden rounded-[1.4rem] bg-gradient-to-r from-white via-white to-orange-50/80 dark:from-slate-900 dark:via-slate-900 dark:to-orange-900/20 px-1 py-2 sm:px-2">
          ...
          <div className="relative z-10 max-w-3xl">
            ...
            <p className="mt-2 max-w-2xl text-sm font-medium text-slate-500 dark:text-slate-400">
              Pantau data pegawai, kehadiran, cuti/izin, reimbursement, banding
              gaji, dan pelanggaran aktif dalam satu halaman.
            </p>
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-2xl ${item.iconBox} dark:bg-slate-700 dark:text-slate-200`}
                  >
                ...
                <div className="mt-5">
                  ...
                  <p
                    className={`mt-1 text-3xl font-extrabold ${item.valueColor} dark:text-white`}
                  >
                    {item.value}
                  </p>
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
            {attendanceCards.map((item) => (
              <button
                ...
                onClick={() =>
                  navigate(
                    `/app/attendance?status=${item.status}&date=${todayDate}`,
                  )
                }
              ...
              >
                <p className="text-sm font-extrabold dark:text-slate-200">
                  {item.label}
                </p>
        ...
        <DashboardSection
          ...
          actionTo={
            activeViolations.length > 10 ? "/app/active-violations" : ""
          }
        >
                ...
                <tbody>
                  {activeViolations.slice(0, 10).map((item) => (
                    <tr key={item.id} className="hover:bg-orange-50/40">
                        ...
                        <button
                          ...
                          onClick={() =>
                            navigate("/app/employees", {
                              state: { employeeId: item.employee_id },
                            })
                          }
                        >
                          {item.employee_name || "-"}
                      ...
                      <td className="max-w-md text-xs font-medium leading-5 text-slate-500">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
          ...
          >
            <div className="mb-4 flex flex-wrap gap-2 text-xs">
              ...
              <span className="rounded-full bg-red-50 px-3 py-1 font-bold text-red-700 dark:bg-red-900/20 dark:text-red-300">
                    ...
                    {leaveManagement.pending_items
                      ...
                      .map((item, idx) => (
                        <tr key={idx} className="hover:bg-orange-50/40">
                          ...
                          <td>
                            <span className="badge badge-sm rounded-full bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300">
                              {item.leave_type}
                            </span>
                          </td>
            ...
            {reimbursementValidation.need_validation?.length > 0 ? (
              <div className="overflow-x-auto rounded-2xl border border-slate-200">
                <table className="table table-sm w-full">
                  ...
                  <tbody>
                    {reimbursementValidation.need_validation
                      .slice(0, 5)
                      .map((item, idx) => (
                        <tr key={idx} className="hover:bg-orange-50/40">
                          <td>
          ...
          >
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {summaryCards.map((item) => (
                <div
                  key={item.label}
                  className={`rounded-2xl border p-4 ${item.box}`}
                >
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
          <DashboardSection
            ...
            actionTo={
              organization.departments?.length > 5 ? "/app/employees" : ""
            }
          >
                  ...
                  <tbody>
                    {organization.departments.slice(0, 5).map((dept, idx) => (
                      <tr key={idx} className="hover:bg-orange-50/40">
                        ...
                        <td className="text-right font-black text-orange-600">
          ...
          <DashboardSection
            ...
            icon={BriefcaseIcon}
            actionTo={
              organization.positions?.length > 5 ? "/app/employees" : ""
            }
          >
                  ...
                  <tbody>
                    {organization.positions.slice(0, 5).map((pos, idx) => (
                      <tr key={idx} className="hover:bg-orange-50/40">
                        ...
                        <td>
                          <span className="badge badge-sm rounded-full bg-orange-50 text-orange-600 dark:bg-orange-900/30 dark:text-orange-300">
                            {pos.level}
                          </span>
                        </td>
                        <td className="text-right font-black text-orange-600">
