...
function EmployeeWarningLetters() {
  ...
  const alphaHistory = (history || []).filter(
    (item) => String(item.status || "").toLowerCase() === "alpha",
  );

  // Pagination logic for alpha history
  const itemsPerPage = 10;
  const totalAlphaPages = Math.ceil(alphaHistory.length / itemsPerPage);
  const startIndex = (alphaPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const paginatedAlphaHistory = alphaHistory.slice(startIndex, endIndex);
              ...
              {alphaHistory.length === 0 ? (
                ...
                paginatedAlphaHistory.map((item, idx) => (
                  <tr key={item.id || idx}>
                    ...
                    <td>
                      {item.note ||
                        (item.status === "alpha" ? "Tidak hadir" : item.status)}
                    </td>
                  </tr>
                ))
              )}
