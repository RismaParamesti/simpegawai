...
function AtasanAttendance() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await atasanApi.getAttendanceRecords({
        month: filters.month,
        year: filters.year,
      });
  ...
  useEffect(() => {
    ...
    if (filters.employeeId !== "all") {
      filteredRecords = filteredRecords.filter(
        (item) => String(item.employee_id) === String(filters.employeeId),
      );
    }
  ...
  useEffect(() => {
    ...
    const activeStillVisible = records.some(
      (item) => String(item.id) === String(activeEditId),
    );
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filters.month, filters.year, filters.employeeId, filters.status]);
  ...
  const summary = records.reduce((acc, item) => {
    ...
    if (item.is_late) {
      ...
      acc.late_minutes =
        (acc.late_minutes || 0) + (Number(item.late_minutes) || 0);
    }
    return acc;
  }, {});
    ...
    <>
      <TitleCard title="Laporan Kehadiran Tim" topMargin="mt-0">
        ...
        <div className="grid md:grid-cols-6 grid-cols-2 gap-4 mb-6">
          <div className="stat rounded-lg border border-sky-200 bg-sky-50 text-sky-900 dark:border-sky-700 dark:bg-sky-900/30 dark:text-sky-100">
            <div className="stat-title text-sky-600 dark:text-sky-300">
              Hadir
            </div>
            <div className="stat-value text-xl text-sky-900 dark:text-sky-100">
              {summary.hadir || 0}
            </div>
          </div>
          <div className="stat rounded-lg border border-blue-200 bg-blue-50 text-blue-900 dark:border-blue-700 dark:bg-blue-900/30 dark:text-blue-100">
            ...
            <div className="stat-value text-xl text-blue-900 dark:text-blue-100">
              {summary.izin || 0}
            </div>
          </div>
          <div className="stat rounded-lg border border-amber-200 bg-amber-50 text-amber-900 dark:border-amber-700 dark:bg-amber-900/30 dark:text-amber-100">
            <div className="stat-title text-amber-600 dark:text-amber-300">
              Sakit
            </div>
            <div className="stat-value text-xl text-amber-900 dark:text-amber-100">
              {summary.sakit || 0}
            </div>
          </div>
          <div className="stat rounded-lg border border-rose-200 bg-rose-50 text-rose-900 dark:border-rose-700 dark:bg-rose-900/30 dark:text-rose-100">
            <div className="stat-title text-rose-600 dark:text-rose-300">
              Alpha
            </div>
            <div className="stat-value text-xl text-rose-900 dark:text-rose-100">
              {summary.alpha || 0}
            </div>
          </div>
          <div className="stat rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-900 dark:border-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-100">
            ...
            <div className="stat-value text-xl text-emerald-900 dark:text-emerald-100">
              {summary.libur || 0}
            </div>
          </div>
          <div className="stat rounded-lg border border-violet-200 bg-violet-50 text-violet-900 dark:border-violet-700 dark:bg-violet-900/30 dark:text-violet-100">
            <div className="stat-title text-violet-600 dark:text-violet-300">
              Terlambat
            </div>
            <div className="stat-value text-xl text-violet-900 dark:text-violet-100">
              {summary.late || 0}
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div>
            <div className="mb-4 flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/10 px-4 py-3 text-sm text-primary">
              <svg
              ...
              <span>
                Ketuk salah satu baris tabel untuk{" "}
                <span className="font-semibold">mengedit data kehadiran</span>
              </span>
            </div>
            <div className="relative overflow-x-auto">
              <table className="table table-zebra">
                ...
                <tbody>
                  ...
                  {records.length === 0 && (
                    <tr>
                      <td colSpan={9} className="text-center opacity-70">
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                              <label className="form-control w-full">
                                ...
                                <input
                                  ...
                                  value={
                                    draft.check_in ?? activeItem.check_in ?? ""
                                  }
                                  onChange={(e) =>
                                    updateRecordField(
                                      activeEditId,
                              ...
                              <label className="form-control w-full md:col-span-2">
                                ...
                                <select
                                  ...
                                  value={
                                    draft.status ?? activeItem.status ?? "hadir"
                                  }
                                  onChange={(e) =>
                                    updateRecordField(
                                      activeEditId,
