...
export const hrApi = {
    ...
    async getActiveWarningLetters(params = {}) {
        try {
            const response = await axios.get('/api/warning-letters/active', { params })
            return response.data || { data: [] }
        } catch (error) {
            throw new Error(parseApiError(error, 'Gagal memuat data pelanggaran aktif'))
        }
    },
