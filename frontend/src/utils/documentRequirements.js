...
function getRequiredDocuments(positionName, basePosition = "") {
  ...
  if (baseNorm.includes("developer")) {
    return (
      POSITION_DOCUMENT_MAPPING["developer"] || { required: [], optional: [] }
    );
  }
  ...
  if (
    (normalized.startsWith("mentor") ||
      normalized.startsWith("project manager")) &&
    baseNorm
  ) {
    ...
    const modifierReq = normalized.startsWith("mentor")
      ? mentorReq
      : projectManagerReq;
    ...
    if (baseReq) {
      const required = [
        ...new Set([
          ...(modifierReq.required || []),
          ...(baseReq.required || []),
        ]),
      ];
      const optional = [
        ...new Set([
          ...(modifierReq.optional || []),
          ...(baseReq.optional || []),
        ]),
      ];
...
const DOCUMENT_FIELD_METADATA = {
  ...
  design_link: {
    ...
    description:
      "Link ke Figma, Behance, Dribbble, atau platform design lainnya",
    accept: "text/plain,.txt",
    maxSize: 1 * 1024 * 1024, // 1MB
    isUrl: true,
  },
