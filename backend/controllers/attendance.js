...
const getLatePolicy = (lateMinutes) => {
  ...
  const isToleratedLate =
    normalizedLateMinutes > 0 &&
    normalizedLateMinutes <= LATE_TOLERANCE_MINUTES;
...
const getEffectiveMaxLeaveDays = (policy, leaveType, cutiKhususOptionKey) => {
  ...
  if (leaveType === "cuti_khusus") {
    const options = Array.isArray(policy.meta?.options)
      ? policy.meta.options
      : [];
    const selectedOption = options.find(
      (option) => option.key === cutiKhususOptionKey,
    );
...
const getCalculatedRemainingLeaveQuota = async (employeeId) => {
  const [quotaResult] = await db.promise().query(
    `SELECT
...
const getSanctionLevelFromAlphaCounts = async ({
  ...
  try {
    ...
    for (const rule of rules) {
      ...
      const normalizedRuleLevel =
        normalizeSanctionLevel(rule.sanction_level) ||
    ...
    console.error(
      "Failed to load attendance_warning_rules:",
      error?.message || error,
    );
  }
...
const evaluateAlphaDisciplineForEmployee = async (employeeId) => {
  const [employeeData] = await db
    .promise()
    .query(`SELECT created_at FROM employees WHERE id = ?`, [employeeId]);
  ...
  const sanctionLabelFromRule =
    sanctionResult && sanctionResult.label
      ? String(sanctionResult.label).trim()
      : null;
  ...
  await db.promise().query(
    `UPDATE employees
         SET alpha_consecutive_days = ?,
             ...
             updated_at = NOW()
         WHERE id = ?`,
    [alphaConsecutiveDays, alphaAccumulatedDays, sanctionLevel, employeeId],
  );
...
const runDailyDisciplineEscalation = async () => {
  ...
  for (const employee of employees) {
    ...
    if (
      result &&
      result.alpha_sanction_level &&
      result.alpha_sanction_level !== ALPHA_SANCTION_LEVEL.NONE
    ) {
      evaluatedWarnings += 1;
    }
  }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const approvedSpecialPermission =
        await getApprovedSpecialPermissionForDate(employeeId, today);
      ...
      if (existingAttendance.length > 0) {
        ...
        const isApprovedHalfDay =
          activeLeave &&
          Number(activeLeave.total_days) > 0 &&
          Number(activeLeave.total_days) < 1;
        const isApprovedIzinTerlambat =
      ...
      const isLeaveTerlambat =
        (approvedSpecialPermission &&
          approvedSpecialPermission.cuti_khusus_option === "terlambat") ||
        (activeLeave &&
          String(activeLeave.cuti_khusus_option) === "terlambat") ||
        (activeLeave && String(activeLeave.leave_type) === "izin_terlambat");

      const lateThresholdTime =
        approvedTimeFromLeave && isLeaveTerlambat
      ...
      if (existingAttendance.length === 0) {
        ...
        await db.promise().query(
          `INSERT INTO attendance (employee_id, date, check_in, status, is_late, late_minutes, created_at) 
                 VALUES (?, ?, ?, 'hadir', ?, ?, NOW())`,
          [employeeId, today, checkInTime, isLate ? 1 : 0, lateMinutes],
        );
      } else {
        ...
        await db.promise().query(
          ...
          [checkInTime, isLate ? 1 : 0, lateMinutes, employeeId, today],
        );
      }
      ...
      try {
        ...
        await logActivity({
          ...
          oldValues:
            existingAttendance.length === 0 ? null : existingAttendance[0],
          newValues: {
            date: today,
      ...
      try {
        await logActivity({
          ...
          role: Array.isArray(req.user?.roles)
            ? req.user.roles[0]
            : req.user?.role || null,
          action: "CREATE",
          module: "attendance",
...
router.get("/today", verifyToken, verifyRole(["pegawai"]), async (req, res) => {
  try {
    ...
    if (attendanceResult.length === 0) {
      ...
      const [activeLeaveResult] = await db.promise().query(
        `SELECT id, leave_type, total_days, time, cuti_khusus_option
                     FROM leave_requests
                     WHERE employee_id = ?
                       AND status = 'approved'
      ...
      if (activeLeaveResult.length > 0) {
        ...
        const isApprovedHalfDay =
          leaveRow.total_days &&
          Number(leaveRow.total_days) > 0 &&
          Number(leaveRow.total_days) < 1;
        const isIzinTerlambat =
          String(leaveRow.leave_type) === "izin_terlambat";
        const canAttendance =
          Boolean(approvedSpecialPermission) ||
          isApprovedHalfDay ||
          isIzinTerlambat;
    ...
    try {
      ...
      const makeBadge = (lvl) => {
        ...
        for (let i = 0; i < m.length; i++)
          hash = (hash << 5) - hash + m.charCodeAt(i);
      ...
      const disciplineWithLabel = {
        ...
        alpha_sanction_badge:
          disciplineSnapshot.alpha_sanction_badge ||
          makeBadge(disciplineSnapshot.alpha_sanction_level),
      };
...
// ============================
// GET ALL EMPLOYEES ATTENDANCE (HR/Atasan)
// ============================
// HR, Atasan melihat absensi semua pegawai
router.get(
  "/all",
  verifyToken,
  verifyRole(["hr", "atasan"]),
  async (req, res) => {
    try {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      query += " GROUP BY e.id, e.employee_code, u.name ORDER BY u.name ASC";
...
// ============================
// UPDATE ATTENDANCE STATUS (HR/Atasan)
// ============================
// HR, Atasan mengubah status absensi (misal: izin, sakit, alpha)
router.put(
  "/:id/status",
  verifyToken,
  verifyRole(["hr", "atasan"]),
    ...
    try {
      ...
      if (managerScope) {
        ...
        if (managerScope.isDirector) {
          ...
          attendanceParams.splice(
            0,
            attendanceParams.length,
            managerScope.managerEmployeeId,
          );
        }
        ...
        if (!attendanceScope.length) {
          return res.status(403).json({
            message: managerScope.isDirector
      ...
      if (isAdminOrAtasanUpdatingHadir) {
        ...
        const isTodayRecord = recordDate.getTime() === todayDate.getTime();
        const isPastRecord = recordDate.getTime() < todayDate.getTime();
        ...
        await db
          .promise()
          .query("UPDATE attendance SET status = ? WHERE id = ?", [status, id]);
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!shouldScopeAsAtasan(req)) {
        return res.status(403).json({
          message:
            "Atasan hanya dapat mengedit absensi tim saat role atasan sedang aktif",
        });
      }
      ...
      const validStatuses = ["hadir", "izin", "sakit", "alpha", "libur"];
      const nextStatus = String(status || existingAttendance[0].status || "")
        .toLowerCase()
        .trim();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const fallbackMaxDays = getEffectiveMaxLeaveDays(
        null,
        leave_type,
        cuti_khusus_option,
      );
      ...
      const maxDaysForPolicy = getEffectiveMaxLeaveDays(
        policy,
        leave_type,
        cuti_khusus_option,
      );
      ...
      if (
        Number(policy.min_days || 1) > 0 &&
        totalDays < Number(policy.min_days || 1)
      ) {
        return res.status(400).json({
      ...
      if (shouldAutoApprove) {
        status = "approved";
        const autoApprovedBy = requesterIsDirector ? null : employeeId;
...
// ============================
// Pegawai melihat leave request sendiri
router.get(
  "/my-leave-requests",
  verifyToken,
  verifyRole(["pegawai"]),
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Leave policy retrieved", data: policy });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ message: "Server error" });
    }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const requesterRoles = req.user.roles || [];
      const adminDirecturScopeToAtasan =
        requesterRoles.includes("admin") && !requesterRoles.includes("hr");
      let managerScope = null;
      ...
      if (!status || !["approved", "rejected"].includes(status)) {
        return res.status(400).json({
          message: "Invalid status. Valid statuses: approved, rejected",
        });
      }
      ...
      const requesterIsDirector = isDirectorLevelPosition(
        requesterPositionName,
      );
      const requesterIsManager = isManagerLevelPosition(requesterPositionName);
