...
const getAllowanceSettings = async () => {
  try {
    ...
    return {
      ...
      late_deduction_percentage: Number(rows[0].late_deduction_percentage),
      alpha_deduction_percentage: Number(rows[0].alpha_deduction_percentage),
    };
  } catch (e) {
...
const getUnpaidLeaveDaysForPeriod = async (
  employeeId,
  periodMonth,
  periodYear,
) => {
...
const isManagerLevelEmployee = (employeeContext) => {
  const level = String(employeeContext?.level || "")
    .toLowerCase()
    .trim();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        parsedBonus < 0 ||
        parsedOtherAllowance < 0 ||
        parsedOtherDeduction < 0
      ) {
        return res.status(400).json({
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const totalReimbursement = Number(
        reimbursements[0]?.total_reimbursement || 0,
      );

      // Hitung komponen gaji
      const grossSalary = baseSalary + allowanceTotal;
      const totalIncome = Number((grossSalary + totalReimbursement).toFixed(2));
      ...
      res.status(200).json({
        ...
        details: {
          ...
          attendance_summary: {
            ...
            total_deductible_absent_days: totalDeductibleAbsentDays,
          },
        },
      });
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      rows.forEach((r, idx) => {
        ...
        doc.text(String(r.employee_code || "-"), 64, y, {
          width: 72,
          align: "left",
        });
        doc.text(String(r.employee_name || "-"), 140, y, {
          width: 160,
          align: "left",
        });
        doc.text(String(r.position_name || "-"), 304, y, {
          width: 120,
          align: "left",
        });
        ...
        doc.text(fmt(r.final_amount || r.net_salary), 680, y, {
          width: 80,
          align: "right",
        });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      doc.roundedRect(startX, 40, pageWidth, 76, 10).fill(COLOR_PRIMARY);
      ...
      doc.roundedRect(startX, y, pageWidth, 122, 8).fill(COLOR_SURFACE);
      doc
      ...
      doc
        ...
        .fontSize(12)
        .text("Identitas Pegawai", startX + 14, y);
      y += 22;
      ...
      doc.roundedRect(startX, y - 6, pageWidth, 26, 6).fill(COLOR_SECTION_BG);
      doc
        .fillColor(COLOR_PRIMARY)
        .fontSize(12)
        .text("Rincian Pendapatan", startX + 10, y);
      y += 18;
      ...
      const incomeRows = [
        ...
        ["Tunjangan Jabatan", formatCurrency(payroll.other_allowance)],
      ...
      incomeRows.forEach(([label, value]) => {
        doc
          .fontSize(10)
          .fillColor("#333333")
          .text(label, startX + 2, y);
        doc
          .fontSize(10)
          .fillColor("#111111")
          .text(value, startX + 260, y, {
            width: pageWidth - 260,
            align: "right",
          });
        y += 16;
      ...
      doc.roundedRect(startX, y - 6, pageWidth, 26, 6).fill(COLOR_SECTION_BG);
      doc
        .fillColor(COLOR_PRIMARY_HOVER)
        .fontSize(12)
        .text("Rincian Potongan", startX + 10, y);
      y += 18;
      ...
      y += 10;
      doc.roundedRect(startX, y, pageWidth, 36, 6).fill(COLOR_PRIMARY_SOFT);
      doc
      ...
      doc
        ...
        .fontSize(12)
        .text(
          formatCurrency(payroll.final_amount || payroll.net_salary),
          startX + 200,
          y + 11,
          {
            width: pageWidth - 212,
    ...
    try {
      ...
      const normalizedPayrollData = payrollData.map((item) => {
        ...
        return {
          ...
          comparison_old_amount: hasApprovedAppeal
            ? previousFinalAmount
            : Number(
                item.take_home_pay ?? item.final_amount ?? item.net_salary ?? 0,
              ),
      ...
      const visiblePayrollData = !hasPrivilegedRole
        ? normalizedPayrollData.filter((item) => {
            ...
            return (
              String(item.appeal_status || "").toLowerCase() === "approved"
            );
          })
  ...
  async (req, res) => {
    try {
      ...
      const [payroll] = await db
        .promise()
        .query("SELECT * FROM payrolls WHERE id = ? AND deleted_at IS NULL", [
          id,
        ]);
  ...
  async (req, res) => {
    try {
      ...
      const [payroll] = await db
        .promise()
        .query(
          "SELECT id, status FROM payrolls WHERE id = ? AND deleted_at IS NULL",
          [id],
        );
      ...
      if (payroll[0].status !== "claimed") {
        return res.status(400).json({
          message:
            "Payroll harus berstatus claimed sebelum dikirim ke rekening",
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (pendingAppealExists) {
        return res.status(400).json({
          message:
            "Cannot claim payroll with pending salary appeal. Wait for HR review.",
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (payroll[0].status !== "draft") {
        return res.status(400).json({
          message: "Hanya slip berstatus draft yang dapat dihapus",
        });
      }
